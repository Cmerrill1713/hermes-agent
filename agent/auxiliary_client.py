...
def _try_openrouter(explicit_api_key: str = None, model: str = None, *, auxiliary_lane: bool = True) -> Tuple[Optional[OpenAI], Optional[str]]:
    free_only, cfg_model = _aux_openrouter_settings()
    or_model = model or cfg_model
    # ``auxiliary.free_only`` gates the AUXILIARY helper lanes (vision, compression,
    # title, memory flush), not the main-model fallback chain. A caller that names an
    # explicit model (a configured fallback entry) is not an auxiliary request; vetoing
    # it silently disables the user's only backup exactly when the primary is down.
    if free_only and auxiliary_lane and not _is_free_model(or_model):
        logger.warning(
...
def _resolve_openrouter_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    client, default = _try_openrouter(
        ...
        model=req.model,
        # An explicit model means a named entry (main-model fallback / user config),
        # not the auxiliary helper lane — auxiliary.free_only must not veto it.
        auxiliary_lane=not req.model,
    )
    if client is None:
